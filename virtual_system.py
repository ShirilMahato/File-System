    ...
    def change_to_subdirectory(self, sub_dir_name):
        for subdir in self.current_directory.subdirs:
            if subdir.name == sub_dir_name:
                ...
                return
        print(
            f"Sub-directory '{sub_dir_name}' not found in current directory.")
    ...
    def chmod(self, permissions, item_name):
        ...
        if item is not None:
            try:
                ...
                print(
                    f"Permissions of '{permissions}' changed to '{item_name}'.")
            except ValueError:
...
def add_directory_to_directory(parent_dir, sub_dir):
    for i in range(parent_dir.subdir_count):
        if parent_dir.subdirs[i].name == sub_dir.name:
            print(
                f"Directory '{sub_dir.name}' already exists in the parent directory.")
            return
    ...
    def create_directory(self, name, parent=None):
        # Default permissions set to 755
        new_dir = Directory(name, 755, parent)
        self.current_directory.subdirs.append(new_dir)
        return new_dir
        ...
        if command_name == "mkdir":
            if len(arguments) > 0:
                ...
                new_directory = Directory(
                    arguments[0], permissions, self.current_directory)
        ...
        for file in dir.files:
            if file.name == file_name:
                ...
                file_path = os.path.join(os.path.abspath(
                    dir.name), file_name)  # Absolute path
                try:
                    os.remove(file_path)
                    print(
                        f"File '{file_name}' deleted from directory '{dir.name}'.")
                except OSError as e:
                    print(f"Error while deleting the file: {e}")
                    print(
                        f"Failed to delete file '{file_name}' at path: {file_path}")
                break
        else:
            print(
                f"File '{file_name}' not found in the directory '{dir.name}'.")
        ...
        for sub_dir in dir.subdirs:
            if sub_dir.name == sub_dir_name:
                ...
                try:
                    ...
                    print(
                        f"Directory '{sub_dir_name}' deleted from directory '{dir.name}'.")
                except OSError as e:
                    print(f"Error while deleting the directory: {e}")
                    print(
                        f"Failed to delete directory '{sub_dir_name}' at path: {sub_dir_path}")
                break
        else:
            print(
                f"Sub-directory '{sub_dir_name}' not found in the directory '{dir.name}'.")
...
def copy_file_to_directory(source_file, target_dir, new_file_name):
    copied_file = File(new_file_name, source_file.content,
                       source_file.permissions)
    add_file_to_directory(target_dir, copied_file)


def copy_directory_to_directory(source_dir, target_dir, new_sub_dir_name):
    copied_dir = Directory(
        new_sub_dir_name, source_dir.permissions, target_dir)
...
def list_directory_contents(dir, detailed):
    ...
    if not has_items:
        ...
        for subdir in dir.subdirs:
            if detailed:
                permissions = format_permissions(
                    subdir.permissions, is_file=False)
                print(
                    f"Sub-directory: {blue_color}{subdir.name}{reset_color} (Created: {time.ctime(subdir.creation_time)}, Permissions: {permissions})")
            else:
...
def append_file_content(file):
    ...
    if permissions_binary & 2:  # Check if the file has write permission
        print(
            f"Enter content to append to file '{file.name}'. Press Ctrl+z followed by enter in the new line to save and exit.")
        appended_content = []
        while True:
            try:
                line = input()
                # +1 for the newline character
                if len(file.content) + len(line) + 1 <= MAX_CONTENT_LENGTH:
                    appended_content.append(line)
                else:
                    print(
                        f"File '{file.name}' is full. Cannot append more content.")
            except EOFError:
        ...
        print(
            f"Permission denied: Cannot append content to file '{file.name}'")
...
def edit_file_content(file):
    ...
    if permissions_binary & 2:  # Check if the file has write permission
        print(
            f"Enter new content for file '{file.name}'. Press Ctrl+z followed by enter in the new line to save and exit.")
        new_content = []
        while True:
            try:
                line = input()
...
def copy_file_or_directory_to_directory(source_item, target_dir, new_name):
    if isinstance(source_item, File):
        copied_file = File(new_name, source_item.content,
                           source_item.permissions, target_dir)
        add_file_to_directory(target_dir, copied_file)
        ...
        for file in source_item.files:
            copied_file = File(file.name, file.content,
                               file.permissions, copied_dir)
            add_file_to_directory(copied_dir, copied_file)
...
def save_project_to_system():
    answer = input(
        "Do you want to save the project to the system? (y/n): ").lower()
    if answer == 'y':
...
def find_item(start_directory, name, path=[]):
    results = []
    for file in start_directory.files:
        if file.name == name:
            results.append(("F", file, path + [file.name]))
    for subdir in start_directory.subdirs:
        if subdir.name == name:
            results.append(("D", subdir, path + [subdir.name]))
        results.extend(find_item(subdir, name, path + [subdir.name]))
    return results
...
#     zip_file_path = os.path.join(current_directory.name, zip_name + ".zip")
#     with zipfile.ZipFile(zip_file_path, 'w') as zipf:
#         for root, _, files in os.walk(directory.name):
#             for file in files:
#                 file_path = os.path.join(root, file)
#                 arcname = os.path.relpath(file_path, directory.name)
...
def copy_file_or_directory_to_same_directory(source_item, new_name):
    if isinstance(source_item, File):
        if find_file_or_directory_in_directory(source_item.parent, new_name):
            print(f"File '{new_name}' already exists in the directory.")
            return
        copied_file = File(new_name, source_item.content,
                           source_item.permissions, source_item.parent)
        add_file_to_directory(source_item.parent, copied_file)
    elif isinstance(source_item, Directory):
        if find_file_or_directory_in_directory(source_item.parent, new_name):
            print(f"Directory '{new_name}' already exists in the directory.")
            return
        copied_dir = Directory(new_name, source_item.permissions, source_item.parent)
        for file in source_item.files:
            copied_file = File(file.name, file.content, file.permissions, copied_dir)
            add_file_to_directory(copied_dir, copied_file)
        for subdir in source_item.subdirs:
            copy_directory_to_directory(subdir, copied_dir, subdir.name)
        add_directory_to_directory(source_item.parent, copied_dir)


def move_file_or_directory_to_same_directory(source_item, new_name=None):
    if isinstance(source_item, File):
        source_item.parent.files.remove(source_item)
        if new_name:
            source_item.name = new_name
        source_item.parent.files.append(source_item)
    elif isinstance(source_item, Directory):
        source_item.parent.subdirs.remove(source_item)
        if new_name:
            source_item.name = new_name
        source_item.parent.subdirs.append(source_item)
...
def execute_command(command, fs):
    ...
    if command_name == "ls":
        list_directory_contents(
            current_directory, "-l" in arguments or "-la" in arguments or "-al" in arguments)
    elif command_name == "mkdir":
        if len(arguments) > 0:
            add_directory_to_directory(
                current_directory, fs.create_directory(arguments[0], current_directory))
        else:
                ...
                if 0 <= permissions <= 777:
                    for file in current_directory.files:
                        ...
                        for subdir in current_directory.subdirs:
                            ...
                            print(
                                f"Item '{arguments[0]}' not found in the current directory.")
                else:
                    print(
                        "Invalid permissions. Please enter a number between 0 and 777.")
            except ValueError:
        ...
        if len(arguments) > 0:
            add_file_to_directory(
                current_directory, fs.create_file(arguments[0], ""))
        else:
        ...
        if len(arguments) > 0:
            for file in current_directory.files:
                ...
                print(
                    f"File '{arguments[0]}' not found in the current directory.")
        else:
    ...
    elif command_name == "cpd":
        if len(arguments) > 1:
            source_item = find_file_or_directory_in_directory(
                current_directory, arguments[0])
            target_dir = find_subdirectory_in_directory(
                current_directory, arguments[1])

            if source_item is not None and target_dir is not None:
                copy_file_or_directory_to_directory(
                    source_item, target_dir, arguments[0])
            else:
                print("Source item or target directory not found.")
        else:
            print("Usage: cp <source_file_or_dir_name> <target_dir_name>")

    elif command_name == "mvd":
        if len(arguments) > 1:
            source_item = find_file_or_directory_in_directory(
                current_directory, arguments[0])
            target_dir = find_subdirectory_in_directory(
        ...
        if len(arguments) > 0:
            ...
            if source_item is not None:
                new_name = arguments[1] if len(arguments) > 1 else None
                copy_file_or_directory_to_same_directory(source_item, new_name)
            else:
                print("Source item not found.")
        else:
            print("Usage: cpd <source_file_or_dir_name> [new_name]")
        ...
        if len(arguments) > 0:
            ...
            if source_item is not None:
                new_name = arguments[1] if len(arguments) > 1 else None
                move_file_or_directory_to_same_directory(source_item, new_name)
            else:
                print("Source item not found.")
        else:
            print("Usage: mvd <source_file_or_dir_name> [new_name]")
        ...
        if len(arguments) > 0:
            ...
            for item_type, item, path in items:
                symbol = "F" if item_type == "F" else "D"
                item_path = "/".join(path)
                print(f"{symbol} {item_path}")
        else:
            print("Usage: find <file_or_dir_name>")
        ...
        item = find_file_or_directory_in_directory(
            current_directory, arguments[0])
        if item:
        ...
        print(f"Command '{command_name}' not found. Use 'help' to see available commands.")
